...
@addons.using_psi4
def test_psi4_hcn_irc(localizer, molecule_engine):
    """
    IRC test with Psi4
    """
...
@addons.using_qchem
def test_qchem_hcn_irc(localizer, molecule_engine):
    """
    IRC test with QChem
    """
...
@addons.using_qchem
def test_qchem_h2o_irc(localizer, molecule_engine):
    """
    IRC test with QChem for the case with two degenerate imaginary modes
    """
    ...
    # Energies of product and reaction should be the same
    assert np.isclose(reac_e, prod_e)

    # Check the end-point energies
    assert np.isclose(reac_e, e_ref)
    assert np.isclose(prod_e, e_ref)
...
def test_gaussian_hcn_irc(localizer, molecule_engine):
    """
...
def test_tera_hcn_irc(localizer, molecule_engine):
    """
